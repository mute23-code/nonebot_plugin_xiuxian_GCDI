...
from nonebot import require, load_all_plugins, get_plugin_by_module_name, load_plugins
...
from .utils import Txt2Img, data_check_conf, check_user_type, get_msg_pic, check_user, send_img_msg
...
async def _(bot: Bot, event: MessageEvent):
    ...
    await send_img_msg(bot, event, msg=result)
...
async def _(bot: Bot, event: MessageEvent):
    ...
    if message == "排行榜" or message == "修仙排行榜" or message == "境界排行榜":
        ...
        await send_img_msg(bot, event, msg=p_rank)
    elif message == "灵石排行榜":
        a_rank = sql_message.stone_top()
        await send_img_msg(bot, event, msg=a_rank)
        ...
        await send_img_msg(bot, event, msg=s_rank)
...
async def _(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    await send_img_msg(bot, event, msg=mes)
